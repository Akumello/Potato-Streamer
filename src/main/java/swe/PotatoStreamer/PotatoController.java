...
public class PotatoController {
	...
	public String manuallyFillDatabase(Model model) throws Exception {
		...
		try {
			DBInteract.addNewMusic(existingUser.getId(), afile.song.getTitle(), afile.song.getArtist(),
					afile.song.getAlbum(), afile.song.getPath());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		try {
			DBInteract.addNewMusic(existingUser.getId(), afile2.song.getTitle(), afile2.song.getArtist(),
					afile2.song.getAlbum(), afile2.song.getPath());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		try {
			DBInteract.addNewMusic(existingUser.getId(), afile3.song.getTitle(), afile3.song.getArtist(),
					afile3.song.getAlbum(), afile3.song.getPath());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public String libraryRender(@ModelAttribute User existingUser, Model model) throws SQLException {
		...
		for (AudioFile audioFile : audioFiles) {
			System.out.println(
					audioFile.song.getArtist() + " " + audioFile.song.getTitle() + " " + audioFile.song.getPath());
		}
